...
module.exports.preBuildDevelopment = async () => {
  ...
  // pine house menus
  const pineHouseMenus = await axios.get(
    envVars.NEXT_PUBLIC_TENANT_API +
      "/api/menus/pine-house-menus?include=nodes.children,parentNodes"
  );

  const pineHouseMenu = dataFormatter.deserialize(pineHouseMenus.data);
  ...
  generateStaticJson("pineHouseMenu.json", pineHouseMenu);
